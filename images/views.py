...
def image_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, "Something wrong with your form")
    else:
...
def image_ranking(request):
    image_ranks = r.zrange('image_ranking', 0, -1, desc=True)[:10]
    ids = [int(id) for id in image_ranks]
    most_viewed = list(Image.objects.filter(id__in=ids))
    most_viewed.sort(key=lambda x: ids.index(x.id))
